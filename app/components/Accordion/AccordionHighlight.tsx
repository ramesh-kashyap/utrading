    ...
    const AccordionHeader = (item:any, _:any, isActive:any) => {
        return(
            <View style={{
                ...
                <Text style={[FONTS.font,FONTS.fontMedium,{color:isActive ? COLORS.title : colors.title,flex:1}]}>{item.title}</Text>
                <View
                    style={[{
                        height:24,
                        width:24,
                ...
                >
                    <Feather  size={18} color={isActive ? COLORS.title : colors.title} name={isActive ? 'minus' : 'plus'}/>
                </View>
            </View>
        )
    }
